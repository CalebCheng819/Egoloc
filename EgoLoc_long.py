...
def extract_local_minima_frames_adaptive(
    ...
    frames = frames_all[mask]; speeds = speeds_all[mask]
    N = len(speeds)
    if N < 4:
        print(f"⚠️ 有效数据太少: {N}")
        return [], []
...
def select_top_n_frames_from_json(json_path, n, frame_index = None, flag = None, receive_flag = None):
    ...
    if frame_index is None:
        ...
        if flag == "feedback":
            valid_frames = [
            (index, speed) for index, speed in data if speed != 0.0 and not math.isnan(speed) and index > frame_index
        ]
            invalid_list = [
            ...
            valid_frames = [
            (index, speed) for index, speed in data if speed != 0.0 and not math.isnan(speed) and speed < frame_index
        ]
            invalid_list = [
...
# Iterate to narrow down the time
    ...
    while minima_indices:
        ...
        candidate_speeds = [all_speeds[np.where(all_frames == i)[0][0]] if np.any(all_frames == i) else 9999 for i in candidate_indices]
                ...
                if description != -1:
                    ...
                    if use_feedback:
                        ...
                        while feedback_count < max_feedback:
                            ...
                            # 采样新帧
                            feedback_count += 1
                            feedback_candidates = [i for i in range(final_frame - feedback_window, final_frame + feedback_window + 1)
                                                  if 0 <= i < total_frames and i not in tried_frames]
                            if not feedback_candidates:
                                break
                            feedback_speeds = [all_speeds[np.where(all_frames == i)[0][0]] if np.any(all_frames == i) else 9999 for i in feedback_candidates]
                            if keyframe_sampling_mode == "adaptive":
...
parser.add_argument('--keyframe_sampling_mode', type=str, default='adaptive', choices=['adaptive', 'random'], help='关键帧索引采样方式: adaptive(速度加权) or random(等概率)')
